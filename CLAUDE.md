...
- **ESLint**: Linter — rules ratchet down over time via [eslint-seatbelt](./contributingGuides/LINTING.md)
...
2. **ESLint**: Run `npx eslint <changed files>` to catch lint errors early. The repo uses eslint-seatbelt (see [LINTING.md](./contributingGuides/LINTING.md)) so CI fails on any new error; pre-existing violations appear as warnings.
